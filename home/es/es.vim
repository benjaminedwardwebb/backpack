...
syn cluster esBody
contains=esString,esBlock,esList,esVariableRef,esBadVariableRef,esPrimitive,esComment,esOperator,esRedirNum,esSpecialChar,esBadSpecialChar,esWrapLine,@esKeyword,

syn region esString matchgroup=esOperator start=/'/ skip=/''/ end=/'/
syn region esBlock  matchgroup=esOperator start=/{/ end=/}/
contains=@esBody
syn region esList   matchgroup=esOperator start=/(/ end=/)/
contains=@esBody
...
syn match esBadSpecialChar
/\(\\[89A-WYZcdg-mo-qsu-wyz]\|\\[Xx]0*\x\@!\|\\0\+\o\@!\)/
